...
export default function BenefitsBlockNew({ id, content }) {
  ...
  // --- CONFIGURATION AVEC NOMS COURTS (CACHE-BUSTER) ---
          ...
          <div className="lg:w-5/12 flex flex-col relative">
            ...
            <div className="flex flex-col gap-[90vh] pb-[40vh] pt-[10vh]">
              {benefitsData.map((benefit, index) => (
                <div key={index} ref={el => observerRefs.current[index] = el} className={`relative pl-20 transition-all duration-700 ease-out ${activeCard === index ? 'opacity-100 blur-0' : 'opacity-20 blur-[2px]'}`}>
                  ...
                  <h3 className="text-3xl md:text-4xl font-playfair font-bold mb-6 text-white">{benefit.title}</h3>
                  <p className="text-gray-300 text-lg leading-relaxed mb-8 font-light">{benefit.description}</p>
