...
export default function HeroBlockResponsive({ id, content }) {
  ...
  const openYouform = () => {
    ...
    if (typeof window !== "undefined" && window.Youform) {
      window.Youform.openPopup("https://app.youform.io/forms/VOTRE_ID_YOUFORM", {
        mode: "popup",
        hideHeader: true,
        hideFooter: true,
        opacity: 85,
  ...
  const openCalendly = () => {
    ...
    if (typeof window !== "undefined" && window.Calendly) {
      window.Calendly.initPopupWidget({
        url: "https://calendly.com/votre-nom/30min", // Remplacez par votre URL Calendly
      })
      return false
    }
  }
  ...
  }, [language]) // Uniquement dépendant de language
    ...
    <section id={id} className={sectionClasses} style={sectionStyle}>
      ...
      <div className="container mx-auto px-4 flex flex-col md:flex-row items-center relative z-10">
        <div className="w-full md:w-1/2 mb-10 md:mb-0 pr-0 md:pr-8 text-center md:text-left">
          <h1 className={`font-playfair font-bold mb-4 ${titleClasses}`}>
          ...
          <h2 className={`font-montserrat font-medium mb-8 text-white ${subtitleClasses}`}>{subtitle}</h2>

          {description && <p className="font-montserrat text-base md:text-lg text-white mb-8">{description}</p>}
